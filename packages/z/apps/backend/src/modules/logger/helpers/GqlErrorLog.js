...
function GqlErrorLog(requestContext) {
    try{
        if (process.env.LOG_GQL_ERRORS == "ON") {
            ...
            let user = (requestContext && requestContext.context && requestContext.context.user) ? requestContext.context.user.username ? requestContext.context.user.username : "anonymous" : "anonymous"
